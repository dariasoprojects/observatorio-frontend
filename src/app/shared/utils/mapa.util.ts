...
export class Mapa {
  ...
  private applyPeruInitialView(): void {
    ...
    this.mapView.goTo({
      ...
      console.warn('No se pudo aplicar la vista inicial del mapa:', err);
    });
  }
  ...
  private restaurarEstadoInicialParcelas(): void {
    ...
    for (const s of subs) {
      const renderer = this.crearRendererSimpleDesdeOriginal((s as any)._rendererOriginal);
  ...
  private crearRendererSimpleDesdeOriginal(orig: any): any | null {
    const sym = orig?.symbol;
    if (!sym) return null;

    const autoSym = this.convertirSimboloServidorAAutocast(sym);
    if (!autoSym) return null;
  ...
  private convertirSimboloServidorAAutocast(sym: any): any | null {
    switch (sym.type) {
      case "esriSFS":
        return {
          type: "simple-fill",
  ...
  agregarBotones() {
    ...
    this.toc_3D.onclick = () => {
      ...
      this.toggle3D().catch((err) => {
        console.warn('No se pudo alternar la vista 2D/3D:', err);
      });
    };
  ...
  async generarTOC(panel: HTMLElement) {
    ...
    this.inicializarPanelTOC(panel);

    const layers = this.map.layers.toArray();

    for (const layer of layers) {
      if (this.debeOmitirCapaTOC(layer)) continue;

      const tocItem = this.crearItemCapaTOC(panel, layer);
      const esCoberturas = await this.renderizarCoberturasTOC(layer, tocItem);

      if (esCoberturas) continue;

      this.configurarToggleCapaTOC(layer, tocItem);
      this.agregarLeyendaUniversalTOC(tocItem);
    }
  }

  private inicializarPanelTOC(panel: HTMLElement): void {
    panel.innerHTML = '';
    panel.style.fontFamily = "Arial, sans-serif";
    panel.style.fontSize = "13px";
    panel.style.position = "relative";
    panel.style.paddingTop = "22px";
    panel.appendChild(this.crearBotonCerrarTOC(panel));
  }

  private crearBotonCerrarTOC(panel: HTMLElement): HTMLDivElement {
  ...
  private debeOmitirCapaTOC(layer: any): boolean {
    return [
      this.capaCluster,
      this.capaClusterPpa,
      this.highlightLayer,
  ...
  private crearItemCapaTOC(panel: HTMLElement, layer: any): any {
    const item = this.crearContenedorItemTOC(layer);
    panel.appendChild(item);

    const rowTop = this.crearFilaSuperiorTOC(item);
    const chk = this.crearCheckboxCapaTOC(rowTop, layer);
    const lbl = this.crearLabelCapaTOC(rowTop, layer);
    const mapImage = layer.type === "map-image" ? (layer as __esri.MapImageLayer) : null;
    const feature = layer.type === "feature" ? (layer as __esri.FeatureLayer) : null;

    this.resaltarParcelasTOC(layer, item, lbl);

    return { item, rowTop, chk, lbl, mapImage, feature };
  }

  private crearContenedorItemTOC(layer: any): HTMLDivElement {
  ...
  private crearFilaSuperiorTOC(item: HTMLDivElement): HTMLDivElement {
  ...
  private crearCheckboxCapaTOC(rowTop: HTMLDivElement, layer: any): HTMLInputElement {
  ...
  private crearLabelCapaTOC(rowTop: HTMLDivElement, layer: any): HTMLSpanElement {
  ...
  private resaltarParcelasTOC(layer: any, item: HTMLDivElement, lbl: HTMLSpanElement): void {
    if (!((layer.title || "").toUpperCase().includes("PARCELAS DE PRODUCTORES"))) return;
  ...
  private async renderizarCoberturasTOC(layer: any, tocItem: any): Promise<boolean> {
    if (layer !== this.capaCoberturas || !tocItem.mapImage) return false;
    ...
    const subsWrap = this.crearContenedorSublayersCoberturas(tocItem.item, tocItem.lbl);
    this.configurarToggleCoberturasTOC(cob, tocItem.chk);

    const legendJson = await this.getCoberturasLegend();
    const legendById = (id: number) => this.buscarLeyendaPorId(legendJson, id);
    const subs = cob.sublayers?.toArray?.() ?? [];

    subs.forEach((s: any) => {
      this.agregarSublayerCoberturaTOC(subsWrap, cob, s, tocItem.chk, legendById);
    });

    return true;
  }

  private crearContenedorSublayersCoberturas(item: HTMLDivElement, lbl: HTMLSpanElement): HTMLDivElement {
    let expanded = true;
  ...
  private configurarToggleCoberturasTOC(cob: __esri.MapImageLayer, chk: HTMLInputElement): void {
    chk.onchange = () => {
      cob.visible = chk.checked;

      if (!chk.checked) {
        this.apagarSublayersCoberturasTOC(cob);
      }
  ...
  private apagarSublayersCoberturasTOC(cob: __esri.MapImageLayer): void {
  ...
  private buscarLeyendaPorId(legendJson: any, id: number): any {
    const arr = legendJson?.layers ?? [];
    return arr.find((x: any) => Number(x.layerId) === id) ?? null;
  }

  private agregarSublayerCoberturaTOC(
    subsWrap: HTMLDivElement,
    cob: __esri.MapImageLayer,
    s: any,
    chk: HTMLInputElement,
    legendById: (id: number) => any
  ): void {
    const row = this.crearFilaSublayerCoberturaTOC(cob, s);
    const subChk = this.crearCheckboxSublayerCoberturaTOC(cob, s, chk);
    const name = this.crearNombreSublayerCoberturaTOC(cob, s, chk);

    row.appendChild(subChk);
    row.appendChild(name);
    this.agregarMiniLeyendaCoberturaTOC(row, legendById(s.id));
    subsWrap.appendChild(row);
  }

  private crearFilaSublayerCoberturaTOC(cob: __esri.MapImageLayer, s: any): HTMLDivElement {
  ...
  private crearCheckboxSublayerCoberturaTOC(
    cob: __esri.MapImageLayer,
    s: any,
    chk: HTMLInputElement
  ): HTMLInputElement {
    ...
    subChk.onchange = () => this.cambiarSublayerCoberturaTOC(cob, s, chk, subChk.checked);
    return subChk;
  }

  private cambiarSublayerCoberturaTOC(
    cob: __esri.MapImageLayer,
    s: any,
    chk: HTMLInputElement,
    checked: boolean
    ...
    if (!checked) {
      s.visible = false;
      s.definitionExpression = "1=0";
      this.actualizarVisibilidadPadreCoberturasTOC(cob, chk);
      this.generarTOC(this.tocContainer);
      return;
    }
  ...
  private actualizarVisibilidadPadreCoberturasTOC(cob: __esri.MapImageLayer, chk: HTMLInputElement): void {
    const quedaAlguno = (cob.sublayers?.toArray?.() ?? []).some(ss => ss.visible);
    if (quedaAlguno) return;

    cob.visible = false;
    chk.checked = false;
  }

  private crearNombreSublayerCoberturaTOC(
    cob: __esri.MapImageLayer,
    s: any,
    chk: HTMLInputElement
  ): HTMLSpanElement {
  ...
  private agregarMiniLeyendaCoberturaTOC(row: HTMLDivElement, li: any): void {
    if (!li?.legend?.length) return;
  ...
  private configurarToggleCapaTOC(layer: any, tocItem: any): void {
    tocItem.chk.onchange = () => {
      if (tocItem.mapImage) {
        const active = this.obtenerSublayerActivoTOC(tocItem.mapImage);
        if (active) active.visible = tocItem.chk.checked;
        tocItem.mapImage.visible = tocItem.chk.checked;
        return;
      }

      layer.visible = tocItem.chk.checked;
    };
  }

  private obtenerSublayerActivoTOC(mil: __esri.MapImageLayer): any {
    const subs = mil.sublayers;
    if (!subs) return null;
    return subs.find(s => s.visible) ?? subs.getItemAt(0) ?? null;
  }

  private agregarLeyendaUniversalTOC(tocItem: any): void {
    const renderer = this.obtenerRendererLeyendaTOC(tocItem);
    if (!renderer) return;

    const legend = this.crearContenedorLeyendaTOC();

    if (this.agregarLeyendaUniqueValueTOC(renderer, legend)) {
      tocItem.rowTop.appendChild(legend);
      return;
    }

    if (this.agregarLeyendaSimpleTOC(renderer, legend)) {
      tocItem.rowTop.appendChild(legend);
    }
  }

  private obtenerRendererLeyendaTOC(tocItem: any): any {
    if (tocItem.feature?.renderer) return tocItem.feature.renderer;
    if (!tocItem.mapImage) return null;

    const active = this.obtenerSublayerActivoTOC(tocItem.mapImage);
    return active?.renderer ?? null;
  }

  private crearContenedorLeyendaTOC(): HTMLDivElement {
  ...
  private agregarLeyendaUniqueValueTOC(renderer: any, legend: HTMLDivElement): boolean {
    if (renderer.type !== "unique-value" || !Array.isArray(renderer.uniqueValueInfos)) return false;

    renderer.uniqueValueInfos.forEach((info: any) => {
      const swatch = this.crearSwatchColorTOC(info.symbol?.color, "14px", "14px");
      if (swatch) legend.appendChild(swatch);
    });

    return true;
  }

  private agregarLeyendaSimpleTOC(renderer: any, legend: HTMLDivElement): boolean {
    const swatch = this.crearSwatchColorTOC(renderer.symbol?.color, "16px", "14px");
  ...
  private crearSwatchColorTOC(color: any, width: string, height: string): HTMLDivElement | null {
    ...
    const r = color.r ?? color[0];
    const g = color.g ?? color[1];
    const b = color.b ?? color[2];
    const a = color.a ?? color[3] ?? 255;
