...
export class SuperficieAgricolaService {
  ...
  getDatosIndicadoresbyDepartamento(depCodigo: string): Observable<IndicadoresSumatoriaResponse> {
    const body = new HttpParams({
      fromObject: {
        f: 'json',
        where:  `INDICE = 'SUPAGRI' AND CAPA = 2 AND UBIGEO LIKE '${depCodigo}%'`,
        outFields: 'DDESCR, PRODUCTORES, HECTAREA, PARCELAS, HECTAREA2',
        returnGeometry: 'false',
      },
    });
  ...
  getDatosIndicadoresbyProvincia(provinciaCodigo: string): Observable<IndicadoresSumatoriaResponse> {
    const body = new HttpParams({
      fromObject: {
        f: 'json',
        where:  `INDICE = 'SUPAGRI' AND CAPA = 3 AND UBIGEO LIKE '${provinciaCodigo}%'`,
        outFields: 'DDESCR, PRODUCTORES, HECTAREA, PARCELAS, HECTAREA2',
        returnGeometry: 'false',
      },
    });
