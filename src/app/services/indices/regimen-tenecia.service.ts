...
export class RegimenTeneciaService {
  ...
  getDatosIndicadoresbyDepartamento(depCodigo: string): Observable<IndicadoresSumatoriaResponse> {
    const body = new HttpParams({
      fromObject: {
        f: 'json',
        where:  `INDICE = 'REGTENE' AND CAPA = 2 AND UBIGEO LIKE '${depCodigo}%'`,
        outFields: 'UBIGEO, DDESCR, PARCELAS',
        returnGeometry: 'false',
      },
    });
  ...
  getDatosIndicadoresbyProvincia(provinciaCodigo: string): Observable<IndicadoresSumatoriaResponse> {
    const body = new HttpParams({
      fromObject: {
        f: 'json',
        where:  `INDICE = 'REGTENE' AND CAPA = 3 AND UBIGEO LIKE '${provinciaCodigo}%'`,
        outFields: 'UBIGEO, DDESCR, PARCELAS',
        returnGeometry: 'false',
      },
    });
