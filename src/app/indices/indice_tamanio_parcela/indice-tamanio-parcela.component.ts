...
export class IndiceTamanioParceComponent implements OnInit {
  ...
  public async cargarDatos(): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.tamanioParcelaService.getDatosIndicadores(),
      );
  ...
  public async cargarDatosByDpto(ubigeo: string): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.tamanioParcelaService.getDatosIndicadoresbyDepartamento(ubigeo),
      );
      const features = response?.features ?? [];
      if (features.length > 0) {
        const { tabla, categorias, valores } =
  ...
  public async cargarDatosByProv(ubigeo: string): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.tamanioParcelaService.getDatosIndicadoresbyProvincia(ubigeo),
      );
      const features = response?.features ?? [];
      if (features.length > 0) {
        const { tabla, categorias, valores } =
