...
export class IndicePrincipalesCultivosComponent implements OnInit {
  ...
  private crearGrafico() {
    ...
    this.chart = Highcharts.chart('container-princult', options);
  }

  public async cargarDatos(): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.principalesCultivoService.getDatosIndicadores(),
      );
  ...
  public async cargarDatosByDpto(ubigeo: string): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.principalesCultivoService.getDatosIndicadoresbyDepartamento(ubigeo),
      );
      const features = response?.features ?? [];
      if (features.length > 0) {
        const { tabla, categorias, valores, categoriasOrdenadas } =
  ...
  public async cargarDatosByProv(ubigeo: string): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.principalesCultivoService.getDatosIndicadoresbyProvincia(ubigeo),
      );
      const features = response?.features ?? [];
      if (features.length > 0) {
        const { tabla, categorias, valores, categoriasOrdenadas } =
