...
export class IndiceCultivosTransitComponent implements OnInit {
  ...
  public async cargarDatos(): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.cultivosTransitoriosService.getDatosIndicadores()
      );
  ...
  public async cargarDatosByDpto(ubigeo: string): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.cultivosTransitoriosService.getDatosIndicadoresbyDepartamento(ubigeo)
      );
      const features = response?.features ?? [];
      if (features.length > 0) {
        const { tabla, categorias, valores, categoriasOrdenadas } =  this.indicesUtil.procesarDatosUbigeoOtros(features);
  ...
  public async cargarDatosByProv(ubigeo: string): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.cultivosTransitoriosService.getDatosIndicadoresbyProvincia(ubigeo)
      );
      const features = response?.features ?? [];
      if (features.length > 0) {
        const { tabla, categorias, valores, categoriasOrdenadas } =  this.indicesUtil.procesarDatosUbigeoOtros(features);
