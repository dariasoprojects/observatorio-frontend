...
export class IndiceFuenteIngresoComponent implements OnInit, AfterViewInit {
  ...
  public async cargarDatos(): Promise<void> {
    ...
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.fuenteIngresoService.getDatosIndicadores()
      );
  ...
  public async cargarDatosByDpto(ubigeo: string): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.fuenteIngresoService.getDatosIndicadoresbyDepartamento(ubigeo)
      );
      const features = response?.features ?? [];
      if (features.length > 0) {
        this.tablaDatos = features.map(f => ({
  ...
  public async cargarDatosByProv(ubigeo: string): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.fuenteIngresoService.getDatosIndicadoresbyProvincia(ubigeo)
      );
      const features = response?.features ?? [];
      if (features.length > 0) {
        this.tablaDatos = features.map(f => ({
