...
export class IndiceTipoActividadComponent implements OnInit, AfterViewInit,OnChanges  {
  ...
  public async cargarDatos(): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.tipoActividadService.getDatosIndicadores()
      );
  ...
  public async cargarDatosByDpto(ubigeo: string): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.tipoActividadService.getDatosIndicadoresbyDepartamento(ubigeo)
      );
      const features = response?.features ?? [];
      if (features.length > 0) {
        const { tabla, categorias, valores } = this.indicesUtil.procesarDatosUbigeo(features);
  ...
  public async cargarDatosByProv(ubigeo: string): Promise<void> {
    try {
      const response: IndicadoresSumatoriaResponse = await firstValueFrom(
        this.tipoActividadService.getDatosIndicadoresbyProvincia(ubigeo)
      );
      const features = response?.features ?? [];
      if (features.length > 0) {
        const { tabla, categorias, valores } = this.indicesUtil.procesarDatosUbigeo(features);
