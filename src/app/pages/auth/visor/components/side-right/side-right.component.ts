...
export class SideRightComponent implements AfterViewInit {
  ...
  private getComponentesRecargables() {
    return [
  ...
  private recargarComponentePorDepartamento(comp: any, codigo: string, dptoNombre: string, usarNombre: boolean) {
    if (!comp) return;

    try {
      if (!codigo || codigo === '00') {
        comp.cargarDatos();
        console.log(`${comp.constructor.name} cargado`);
        return;
      }

      comp.cargarDatosByDpto(usarNombre ? dptoNombre : codigo);
      console.log(`${comp.constructor.name} cargado`);
    } catch (err) {
      console.warn(`Error en ${comp.constructor.name}:`, err);
    }
  }

  private recargarComponentePorProvincia(comp: any, codigoProv: string, codigoDpto: string, nombreProv: string | undefined, usarNombre: boolean) {
    if (!comp) return;

    try {
      if (codigoProv !== '00') {
        comp.cargarDatosByProv(usarNombre ? nombreProv : codigoProv);
        return;
      }
  ...
  recargarComponentesPorDepartamento(codigo: string) {
    ...
    this.getComponentesRecargables().forEach(c =>
      this.recargarComponentePorDepartamento(c.ref, codigo, dptoNombre, c.usarNombre)
    );
  }
  ...
  recargarComponentesPorProvincia(codigoProv: string, codigoDpto: string, nombreProv?: string) {
    this.getComponentesRecargables().forEach(c =>
      this.recargarComponentePorProvincia(c.ref, codigoProv, codigoDpto, nombreProv, c.usarNombre)
    );
  }
