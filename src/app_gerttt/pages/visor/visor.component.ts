...
export class VisorComponent implements OnInit {
  ...
  async ejecutarConsulta(event: Event): Promise<void> {
    ...
    await this.cargarInformacionDepartamento(departamento);

  }

  private async cargarInformacionDepartamento(departamento: string): Promise<void> {
    const departamentoNormalizado = this.getNombreDepartamento(departamento);
    ...
    const cargas = [
      { nombre: 'indicePadronProdComponent', componente: this.indicePadronProdComponent, valor: departamento, mensaje: 'indicePadronProdComponent cargados' },
      { nombre: 'indiceCentrosEmpadronamiento', componente: this.indiceCentrosEmpadronamiento, valor: departamentoNormalizado, mensaje: 'indiceCentrosEmpadronamiento cargados' },
      { nombre: 'indiceTipoActividadComponent', componente: this.indiceTipoActividadComponent, valor: departamento, mensaje: 'indiceTipoActividadComponent cargados' },
      { nombre: 'indiceNivelEstudioComponent', componente: this.indiceNivelEstudioComponent, valor: departamento, mensaje: 'indiceNivelEstudioComponent cargado' },
      { nombre: 'indiceSegunRegionNaturalComponent', componente: this.indiceSegunRegionNaturalComponent, valor: departamento, mensaje: 'indiceSegunRegionNaturalComponent cargado' },
      { nombre: 'indiceFuenteIngresoComponent', componente: this.indiceFuenteIngresoComponent, valor: departamento, mensaje: 'indiceFuenteIngresoComponent cargado' },
      { nombre: 'indiceGeneroComponent', componente: this.indiceGeneroComponent, valor: departamento, mensaje: 'indiceGeneroComponent cargado' },
      { nombre: 'indiceTipoOrgComponent', componente: this.indiceTipoOrgComponent, valor: departamento, mensaje: 'indiceTipoOrgComponent cargado' },
      { nombre: 'indiceSuperfiAgriComponent', componente: this.indiceSuperfiAgriComponent, valor: departamento, mensaje: 'indiceSuperfiAgriComponent cargado' },
      { nombre: 'indiceSuperfiSembComponent', componente: this.indiceSuperfiSembComponent, valor: departamento, mensaje: 'indiceSuperfiSembComponent cargado' },
      { nombre: 'indiceTamanioParceComponent', componente: this.indiceTamanioParceComponent, valor: departamento, mensaje: 'indiceTamanioParceComponent cargado' },
      { nombre: 'indiceRegimenTenenComponent', componente: this.indiceRegimenTenenComponent, valor: departamento, mensaje: 'indiceRegimenTenenComponent cargado' },
      { nombre: 'indicePrincipalesCultivosComponent', componente: this.indicePrincipalesCultivosComponent, valor: departamento, mensaje: 'indicePrincipalesCultivosComponent cargado' },
      { nombre: 'indiceCultivosTransitComponent', componente: this.indiceCultivosTransitComponent, valor: departamento, mensaje: 'indiceCultivosTransitComponent cargado' },
      { nombre: 'indiceCultivosPermaComponent', componente: this.indiceCultivosPermaComponent, valor: departamento, mensaje: 'indiceCultivosPermaComponent cargado' }
    ];

    for (const carga of cargas) {
      await this.cargarIndicePorDepartamento(carga);
    }
  }

  private async cargarIndicePorDepartamento(carga: {
    nombre: string;
    componente?: { cargarDatosByDpto: (departamento: string) => Promise<void> | void };
    valor: string;
    mensaje: string;
  }): Promise<void> {
    if (!carga.componente) return;

    try {
      await carga.componente.cargarDatosByDpto(carga.valor);
      console.log(carga.mensaje);
    } catch (err) {
      console.warn(` Error en ${carga.nombre}:`, err);
    }
  }
  ...
  ejecutarConsultaProv(event: Event): void {
    ...
    this.cargarInformacionProvincia(provincia, provincia2);

  }

  private cargarInformacionProvincia(provincia: string, provinciaNombre: string): void {
    this.mapa.queryByProvincia(provincia)
      .then(() => console.log(`Consulta realizada para el departamento: ${provincia}`))
      .catch(err => console.error('Error al realizar la consulta:', err));

    const cargas = [
      { componente: this.sumatoriasComponent, valor: provincia },
      { componente: this.indicePadronProdComponent, valor: provincia },
      { componente: this.indiceCentrosEmpadronamiento, valor: provinciaNombre },
      { componente: this.indiceTipoActividadComponent, valor: provincia },
      { componente: this.indiceNivelEstudioComponent, valor: provincia },
      { componente: this.indiceSegunRegionNaturalComponent, valor: provincia },
      { componente: this.indiceFuenteIngresoComponent, valor: provincia },
      { componente: this.indiceGeneroComponent, valor: provincia },
      { componente: this.indiceTipoOrgComponent, valor: provincia },
      { componente: this.indiceSuperfiAgriComponent, valor: provincia },
      { componente: this.indiceSuperfiSembComponent, valor: provincia },
      { componente: this.indiceTamanioParceComponent, valor: provincia },
      { componente: this.indiceRegimenTenenComponent, valor: provincia },
      { componente: this.indicePrincipalesCultivosComponent, valor: provincia },
      { componente: this.indiceCultivosTransitComponent, valor: provincia },
      { componente: this.indiceCultivosPermaComponent, valor: provincia }
    ];

    cargas.forEach(carga => this.cargarComponentePorProvincia(carga.componente, carga.valor));
  }

  private cargarComponentePorProvincia(
    componente: { cargarDatosByProv: (provincia: string) => void } | undefined,
    provincia: string
  ): void {
    componente?.cargarDatosByProv(provincia);
  }
